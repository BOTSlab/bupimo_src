...
DerivedImgData::DerivedImgData(Img &inImg) :
    img(inImg),
    keys(NULL)//, frontOrderedKeys(NULL), backOrderedKeys(NULL)
{
//    staticWidth = width;
}

DerivedImgData::DerivedImgData(const DerivedImgData &other) {
    deepCopy(other);
}

DerivedImgData& DerivedImgData::operator=(const DerivedImgData &other) {
    destroy();
    deepCopy(other);
}

DerivedImgData::~DerivedImgData() {
    destroy();
}

/*
vector<Keypoint*>& DerivedImgData::getKeypoints() {
    if (keys != NULL)
        
    if (!ssDerivedImgData->hasKeypoints())
        extractor.extract(*inSS, ssDerivedImgData->getKeypoints());
        
    return keypoints;
}
*/

void DerivedImgData::destroy() {
    SiftExtractor::clearKeys(keypoints);
}

/*
void DerivedImgData::deepCopy(const DerivedImgData &other) {
    int nKeys = (int) other.keypoints.size();
    for (int i=0; i<nKeys; i++) {
        Keypoint *copy = new Keypoint(*other.keypoints[i]);
...
//vector<Keypoint*> *frontOrderedKeys;
//vector<Keypoint*> *backOrderedKeys;

/*
void DerivedImgData::getFrontOrderedKeys() {
    ssImg = SS;
...
void FBAlg::computeForB(bool computingF,
    ...
    */
    ...
    if (orderFilter && computingF) {
        frontOrderFilter(ssKeys, cvKeys, matches);
