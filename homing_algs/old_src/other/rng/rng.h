...
class RNG
{
 ...
 public:
  RNG() : /*BEGIN - AV*/inited(0)/*END - AV*/ { init(); zigset(); }
  RNG(ulong x_) : 
    z(x_), w(x_), jsr(x_), jcong(x_) /*BEGIN - AV*/, inited(0)/*END - AV*/{ zigset(); }
  RNG(ulong z_, ulong w_, ulong jsr_, ulong jcong_ ) : 
    z(z_), w(w_), jsr(jsr_), jcong(jcong_) /*BEGIN - AV*/, inited(0)/*END - AV*/{ zigset(); }
  ~RNG() { }
